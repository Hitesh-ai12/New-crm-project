...
export default {
  ...
  computed: {
    sortedSignatures() {
      return [...this.signatures].sort((a, b) => b.is_default - a.is_default);
    }
  },
  mounted() {
  ...
  methods: {
    ...
    async saveSignature() {
      ...
      try {
        if (this.newSignature.id) {
          await axios.put(`/api/signatures/${this.newSignature.id}`, this.newSignature);
          this.showToast('Signature updated successfully!', 'success');
        } else {
          await axios.post('/api/signatures', this.newSignature);
    ...
    async updateDefaultSignature() {
      try {
        await axios.post('/api/signatures/set-default', { id: this.defaultSignature });
        await this.fetchSignatures(); 
        this.showToast('Default signature updated successfully!', 'success');
      } catch (error) {
        console.error('Error setting default signature:', error);
        this.showToast('Error setting default signature.', 'error');
      }
    },
